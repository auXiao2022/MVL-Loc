...
scene_text = {}
scene_text["chess"] = model.encode_text(clip.tokenize("Task: Camera Pose Regression for chess scene").to(device))
scene_text["fire"] = model.encode_text(clip.tokenize("Task: Camera Pose Regression for fire scene").to(device))
scene_text["heads"] = model.encode_text(clip.tokenize("Task: Camera Pose Regression for heads scene").to(device))
scene_text["office"] = model.encode_text(clip.tokenize("Task: Camera Pose Regression for office scene").to(device))
scene_text["pumpkin"] = model.encode_text(clip.tokenize("Task: Camera Pose Regression for pumpkin scene").to(device))
scene_text["redkitchen"] = model.encode_text(clip.tokenize("Task: Camera Pose Regression for redkitchen scene").to(device))
scene_text["stairs"] = model.encode_text(clip.tokenize("Task: Camera Pose Regression for stairs scene").to(device))

#text = clip.tokenize(Global_text).to(device)
#global_feature = model.encode_text(text)
feature_extractor = model

atloc = AtLoc(feature_extractor, droprate=opt.train_dropout, pretrained=True, lstm=opt.lstm,scene_feature= scene_text) #text_feature=global_feature
