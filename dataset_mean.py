...
from dataloaders import RobotCar, SevenScenes
...
elif opt.dataset == 'CambridgeLandmarks': #'Robotcar' CambridgeLandmarks
    dset = SevenScenes(**kwargs)
elif opt.dataset == 'Robotcar': #'Robotcar' CambridgeLandmarks
...
mean_p1 = np.asarray([np.sum(acc[0])])
mean_p2 = np.asarray([np.sum(acc[1])])
mean_p3 = np.asarray([np.sum(acc[2])])
