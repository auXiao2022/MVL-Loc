...
class SevenScenes(data.Dataset):
    def __init__(self, scene, data_path, train, transform=None, target_transform=None, mode=0, seed=7, real=False, skip_images=False, vo_lib='orbslam'):
        self.mode = mode #0
        self.transform = transform #resize,ramdomcrop
        self.target_transform = target_transform#none
        self.skip_images = skip_images#false
        np.random.seed(seed)

        # directories
        data_dir = osp.join(data_path, '7Scenes', scene)
        # data_dir = osp.join(data_path, 'CambridgeLandmarks', scene)
        ...
        with open(split_file, 'r') as f:
            seqs = [int(l.split('sequence')[-1]) for l in f if not l.startswith('#')]#[1,2,4,6]

        # read poses and collect image names
        self.c_imgs = []
        self.d_imgs = []
        self.gt_idx = np.empty((0,), dtype=int)#np.
        ps = {}
        vo_stats = {}
        gt_offset = int(0)
        for seq in seqs:
            seq_dir = osp.join(data_dir, 'seq-{:02d}'.format(seq)) #'seq-{:02d}'
            ...
            if real:
                ...
                frame_idx = np.array(range(len(p_filenames)), dtype=int)#np.
            ...
            c_imgs = [osp.join(seq_dir, 'frame-{:06d}.color.png'.format(i)) #frame-{:06d}.color.png'
                      for i in frame_idx]
...
class RobotCar(data.Dataset):
    def __init__(self, scene, data_path, train, transform=None, target_transform=None, real=False, skip_images=False, seed=7, undistort=False, vo_lib='stereo'):
        ...
        for seq in seqs:
            ...
            self.imgs.extend([osp.join(seq_dir, 'stereo', 'centre', '{:d}.png'.format(t)) for t in ts[seq]]) #centre_processed
